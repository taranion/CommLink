...
public class CustomLoggerFinder extends LoggerFinder {
	...
	public Logger getLogger(String name, Module module) {
		...
		if (name.startsWith("prelle")) {
			...
			ret = new ConsoleLogger(name, Level.WARNING);
		} else if (name.startsWith("de.rpgframework.shadowrun.chargen.enhance") || name.startsWith("de.rpgframework.shadowrun6.chargen6.skill")) {
			ret = new ConsoleLogger(name, Level.DEBUG);
